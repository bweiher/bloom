...
bloom <- function(datatable, cols, retain_all=TRUE, fn) {
  ...
  for(z in seq_along(1:nrow(grid))){
    ...
    for(g in seq_along(chr2)) {  # create filter conditions for this row in the grid
      if(grid[[g]][z] == 'All' || is.na(grid[[g]][z])) chr2[[g]] <- ''
      else chr2[[g]] <- paste0(chr[g], "'", grid[[g]][z], "'")
    }
    ...
    if(filter_condition=='') agg_dt <- fn(datatable[,])
    else agg_dt <- fn(datatable[eval(parse(text=filter_condition)),])
